...
export function ondrag<T>(
  pageY: number,
  dragTarget: HTMLElement | null,
  dropTarget: HTMLElement | null,
  data: TreeData<T>[],
  dropAllowed?: (dropData: DropData<T>) => boolean,
  next?: () => void
) {
  if (dropTarget) {
    ...
    if (dropTargetPathString) {
      ...
      if (targetData.state.dropPosition !== position) {
        ...
        targetData.state.dropAllowed = dropAllowed
...
export function ondrop<T>(
  target: HTMLElement,
  dragTarget: HTMLElement | null,
  data: TreeData<T>[],
  next: (dropData: DropData<T>) => void
) {
  const sourcePath = dragTarget!.dataset.path!.split(",").map(s => +s);
