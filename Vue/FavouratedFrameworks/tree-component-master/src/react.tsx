...
> {
  ...
  render() {
    ...
    if (this.props.data.children) {
      const nodesElement: JSX.Element[] = this.props.data.children.map(
        (child, i) => (
          <Node
            data={child}
            key={i}
            ...
            toggle={(eventData: common.EventData<T>) =>
              this.ontoggle(eventData)}
            change={(eventData: common.EventData<T>) =>
    ...
    const checkboxElement = this.props.checkbox ? (
      <i className={this.checkboxClassName} role="presentation" />
    ) : null;
    const iconElement =
      this.props.data.icon !== false ? (
        <i className={this.iconClassName} role="presentation" />
      ) : null;
    const markerElement = this.hasMarker ? (
      <span className={this.markerClassName}>&#160;</span>
    ) : null;
    const contextmenu =
      this.props.data.contextmenu && this.contextmenuVisible
        ? React.createElement(
            this.props.data.contextmenu as React.ComponentClass<{
              data: common.ContextMenuData<T>;
            }>,
            { data: this.contextmenuData }
          )
        : null;
    const text = this.props.data.component
      ? React.createElement(
          this.props.data.component as React.ComponentClass<{
            data: common.TreeData<T>;
          }>,
    ...
    return (
      <li role="treeitem" className={this.nodeClassName} id={this.id}>
        ...
        />
        <a
          className={this.anchorClassName}
  ...
  private get hasMarker() {
    return (
      ...
      this.props.data.state.dropPosition !== common.DropPosition.empty
    );
  }
  ...
  private ontoggle(eventData?: common.EventData<T>) {
    if (eventData) {
      ...
      if (
        this.props.data.state.openable ||
  ...
  private oncontextmenu(e: React.MouseEvent<HTMLAnchorElement>) {
    ...
    this.setState({
      ...
      contextmenuStyle: this.contextmenuStyle,
    });
    e.preventDefault();
    return false;
  }
...
> {
  ...
  render() {
    const childrenElement = this.props.data.map((child, i) => (
      <Node
        ...
        change={(data: common.EventData<T>) => this.onchange(data)}
      />
    ));
    return (
  ...
  private get rootClassName() {
    return common.getRootClassName(
      this.props.checkbox,
      this.props.size,
      this.props.theme
    );
  ...
  private canDrop(event: React.DragEvent<HTMLElement>) {
    return (
      ...
      (event.target as HTMLElement).dataset.path
    );
  }
  private ontoggle(eventData: common.EventData<T>) {
