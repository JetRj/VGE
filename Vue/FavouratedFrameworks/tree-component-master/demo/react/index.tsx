  ...
  <button
    onClick={e => {
      const parent = getNodeFromPath(
        props.data.root,
        props.data.path.slice(0, props.data.path.length - 1)
      );
      const children =
        parent && parent.children ? parent.children : props.data.root;
...
const CustomNode: React.StatelessComponent<{
  data: TreeData<Value>;
}> = props => (
  <span>
    <span style={{ color: "red" }}>{props.data.value!.id}</span>
    {props.data.text}
  </span>
);
...
> {
  ...
  render() {
    return (
      <div>
        <a
          href="https://github.com/plantain-00/tree-component/tree/master/demo/react/index.tsx"
          target="_blank"
        >
  ...
  private change(eventData: EventData<Value>) {
    this.selectedId = eventData.data.state!.selected
      ? null
      : eventData.data.value!.id;
