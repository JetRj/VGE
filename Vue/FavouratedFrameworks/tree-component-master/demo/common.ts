...
export function setParentsSelection(tree: TreeData<Value>[], path: number[]) {
  ...
  for (const parent of parents) {
    parent.state.selected = parent.children.every(
      child => child.state.selected
    );
  }
}
...
export function canMove(dropData: DropData<Value>) {
  ...
  return (
    dropData.targetData.state.dropPosition !== DropPosition.inside &&
...
export function move(dropData: DropData<Value>, treeData: TreeData<Value>[]) {
  ...
  const sourceParent = getNodeFromPath(
    treeData,
    dropData.sourcePath.slice(0, dropData.sourcePath.length - 1)
  );
  const sourceChildren =
    sourceParent && sourceParent.children ? sourceParent.children : treeData;
  ...
  if (dropData.targetData.state.dropPosition === DropPosition.inside) {
    ...
    const targetParent = getNodeFromPath(
      treeData,
      dropData.targetPath.slice(0, dropData.targetPath.length - 1)
    );
    const targetChildren =
      targetParent && targetParent.children ? targetParent.children : treeData;
